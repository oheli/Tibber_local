...
class TibberLocalDataUpdateCoordinator(DataUpdateCoordinator):
    def __init__(self, hass: HomeAssistant, config_entry):
        ...
        the_pwd = config_entry.options.get(
        ...
        node_num = int(
            config_entry.options.get(
                CONF_NODE_NUMBER, config_entry.data.get(CONF_NODE_NUMBER, 1)
            )
        )

        # ignore parse errors is only in the OPTIONS (not part of the initial setup)
        ignore_parse_errors = bool(
            config_entry.options.get(CONF_IGNORE_READING_ERRORS, False)
        )
    ...
    async def init_on_load(self):
        try:
            ...
            _LOGGER.info(
                f"after init - found OBIS entries: '{self.bridge._obis_values}'"
            )
...
class TibberLocalBridge:
    ...
    def __init__(
        ...
        if websession is not None:
            ...
            self.url_mode = (
                f"http://admin:{pwd}@{host}/node_params.json?node_id={node_num}"
            )
        self._com_mode = com_mode
        self.ignore_parse_errors = False
        if options and "ignore_parse_errors" in options:
    ...
    async def detect_com_mode(self):
        ...
        _LOGGER.debug(
            f"detect_com_mode: after detect_com_mode_from_node_param27 mode is: {self._com_mode}"
        )
    ...
    async def _check_modes_internal(self, mode_1: int, mode_2: int):
        ...
        if len(self._obis_values) > 0:
            ...
            _LOGGER.debug(
                f"detect_com_mode 1 is {self._com_mode}: {mode_1} failed - will try to read {mode_2}"
            )
            await self.read_tibber_local(mode_2, False, log_payload=True)
            if len(self._obis_values) > 0:
                self._com_mode = mode_2
                _LOGGER.debug(
                    f"detect_com_mode 2 SUCCESS -> _com_mode: {self._com_mode}"
                )
            else:
                _LOGGER.debug(
                    f"detect_com_mode 2 is {self._com_mode}: {mode_1} failed and {mode_2} failed"
                )
                pass
    ...
    async def detect_com_mode_from_node_param27(self):
        try:
            ...
            async with self.websession.get(
                self.url_mode, ssl=False, timeout=10.0
            ) as res:
                try:
                    ...
                    if res.status == 200:
                        ...
                        for a_parm_obj in json_resp:
                            if (
                                "param_id" in a_parm_obj
                                and a_parm_obj["param_id"] == 27
                                or "name" in a_parm_obj
                                and a_parm_obj["name"] == "meter_mode"
                            ):
    ...
    async def read_tibber_local(
        self, mode: int, retry: bool, log_payload: bool = False
    ):
        async with self.websession.get(self.url_data, ssl=False, timeout=10.0) as res:
            try:
                ...
                if res.status == 200:
                    ...
                    if res is not None:
                        _LOGGER.warning(
                            f"access to bridge failed with code {res.status} - res: {res}"
                        )
                    else:
    ...
    def check_first_six_parts_for_digits_or_last_is_none(
        self, parts: list[str]
    ) -> bool:
        return (
            self.ONLY_DIGITS.match(parts[1]) is not None
            and self.ONLY_DIGITS.match(parts[2]) is not None
            and self.ONLY_DIGITS.match(parts[3]) is not None
            and self.ONLY_DIGITS.match(parts[4]) is not None
            and self.ONLY_DIGITS.match(parts[5]) is not None
            and (parts[6] is None or self.ONLY_DIGITS.match(parts[6]) is not None)
        )
    ...
    async def read_plaintext(self, plaintext: str, retry: bool, log_payload: bool):
        try:
                ...
                try:
                    ...
                    if len(parts) == 9:
                        if self.check_first_six_parts_for_digits_or_last_is_none(parts):
                            int_obc = IntBasedObisCode(
                                parts, not self.ignore_parse_errors
                            )
                            value = parts[7]
                            unit = None
                            if "*" in value:
                        ...
                        if parts[0] == "!":
                            ...
                            if not self.ignore_parse_errors:
                                _LOGGER.debug(
                                    f"unknown entry: {parts[0]} (line: '{a_line}')"
                                )
                        # else:
            ...
            if not self.ignore_parse_errors:
                _LOGGER.warning(
                    f"Exception {exc} while process data - plaintext: {plaintext}"
                )
            if retry:
    ...
    async def read_sml(self, payload: bytes, retry: bool, log_payload: bool):
        ...
        try:
            ...
            if sml_frame is None:
                ...
                self._obis_values: dict[ObisCode, SmlListEntry] = {}
                self._obis_values_by_short = {}
            ...
            if not self.ignore_parse_errors:
                _LOGGER.warning(
                    f"Exception {exc} while parse data - payload: {payload}"
                )
            if retry:
                await asyncio.sleep(2.5)
                await self.read_tibber_local(mode=MODE_3_SML_1_04, retry=False)
    ...
    def _get_str_internal(self, key):
        if key in self._obis_values:
            obis = self._obis_values.get(key)
            if not obis:
                raise Exception("`obis` is None. This should not be reached?")
            else:
                return obis.value
    ...
    def attr0100010800ff_status(self) -> float:
        if "0100010800ff" in self._obis_values and hasattr(
            self._obis_values.get("0100010800ff"), "status"
        ):
